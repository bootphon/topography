...
def main(config: CIFARConfig) -> None:
    ...
    split_train = int(np.floor(config.train_proportion * (num_train - split)))
    train_idx, val_idx = indices[split : split + split_train], indices[:split]
    train_sampler = SubsetRandomSampler(train_idx.numpy())
    val_sampler = SubsetRandomSampler(val_idx.numpy())
    ...
    print(
        f"TRAIN LENGTH {len(train_loader)}, VAL LENGTH {len(val_loader)}, TEST LENGTH {len(test_loader)}"
    )
