...
class SpeechCommandsConfig:
    ...
    train_proportion: float = 1.0  # Proportion of the training data kept
...
def main(config: SpeechCommandsConfig) -> None:
    ...
    split = int(np.floor(config.train_proportion * num_train))
    train_sampler = SubsetRandomSampler(indices[:split].numpy())
